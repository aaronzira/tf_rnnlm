...
flags.DEFINE_float("threshold", 20, "cutoff for compared value")
flags.DEFINE_string("compare", "", "gt or lt")
      ...
      if train:
        ...
        if linebyline:
          ...
          while True:
            ...
            ## adjusting prediction mode to find problematic words
            # Prediction mode
            if predict:
            ...
            # ppl or loglikes
              ...
              #_#print(biggest/float(avg))
              #_#print(biggest-avg)

              #_#print(" ".join(sentence))
              #_#print(*(word[:7] for word in sentence),sep='\t')
              #_#print("",*[float("{:.2f}".format(num)) for num in o],sep='\t')

              #_#diffs = [float(val2)-val1 for val1,val2 in zip(o[:-1],o[1:-1])] #o,o[1:]
              #_#print("","",*["{:.4f}".format(diff) for diff in diffs],sep='\t')

              #_#print(np.var(diffs))
              #_#var_of_diffs = np.var(diffs)

              #_#print("{:.2f}\t{:.2f}\t{:.2f}".format(max(diffs),max(o),max(o)-max(diffs)))

            #_#if FLAGS.compare == "gt":
            #_#    if thing_to_compare > FLAGS.threshold:
            #_#        print("{:.2f}".format(thing_to_compare))
            #_#elif FLAGS.compare == "lt":
            #_#    if thing_to_compare < FLAGS.threshold:
            #_#        print("{:.2f}".format(thing_to_compare))
            #_#else:
            #_#    print("{:.2f}".format(thing_to_compare))
